/**
 * FreeFinCalc.net — Page-Level Metadata for All 124 Calculators
 * node add_metadata_all.js
 *
 * Adds a generateMetadata() export to every calculator page.
 * This gives each page a unique <title> and <meta description>
 * which is the single biggest on-page SEO improvement possible.
 *
 * Uses Next.js 14 App Router generateMetadata pattern.
...
// ── Inject generateMetadata into each page ───────────────────────────────────
let updated = 0
let skipped = 0
let notFound = 0
...
for (const [route, title, description] of pages) {
  ...
  // Build the metadata export — must be outside 'use client' component
  // In Next.js 14 App Router, generateMetadata must be in a server component
  // Since our pages are 'use client', we create a metadata.js file alongside
  const metadataContent = `// Auto-generated metadata for SEO
...
`
  // Write metadata.js next to page.js
  const metaPath = path.join('app', route, 'metadata.js')
  fs.writeFileSync(metaPath, metadataContent, 'utf8')

  // Now update page.js to import and re-export metadata
  // Add export { metadata } at the top after 'use client'
  if (content.startsWith("'use client'")) {
    content = `'use client'\nexport { metadata } from './metadata'\n` + content.slice("'use client'\n".length)
    fs.writeFileSync(filePath, content, 'utf8')
    updated++
  } else {
    // Just prepend the metadata export
    content = `export { metadata } from './metadata'\n` + content
    fs.writeFileSync(filePath, content, 'utf8')
    updated++
  }
}
...
════════════════════════════════════════════════════
  ...
  ⏭️  Skipped : ${skipped} (already have metadata)
  ⚠️  Missing : ${notFound} (page files not found)
════════════════════════════════════════════════════
  Each calculator now has unique:
  - <title> tag
  - <meta description>
  - Open Graph tags
  - Twitter card tags
