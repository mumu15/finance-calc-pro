/**
 * Fix: Cannot use 'use client' + generateStaticParams in same file
 * Solution: page.js = server component (has generateStaticParams)
 *           StateMortgageClient.js = client component (has 'use client' + useState)
 * 
 * node fix_state_pages.js
 */
...
// ── FILE 1: page.js — SERVER component, has generateStaticParams ──────────
const pageCode = `import states from '../../../../data/states'
...
// ── FILE 2: StateMortgageClient.js — CLIENT component, has useState ───────
...
fs.writeFileSync(path.join(stateDir, 'StateMortgageClient.js'), clientCode, 'utf8')
console.log('✅ page.js — server component with generateStaticParams')
console.log('✅ StateMortgageClient.js — client component with useState')
...
`  <url>
  ...
  console.log('✅ sitemap.xml updated — 50 state pages added')
} else {
  console.log('ℹ️  Sitemap already has state pages or not found')
}

console.log('\nDeploy:\n  git add -A\n  git commit -m "feat: 50 state mortgage pages"\n  vercel --prod')
