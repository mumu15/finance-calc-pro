/**
 * FreeFinCalc.net — Fix Header
 * node fix_header.js
 *
 * 1. Removes the desktop category quick-links bar
 * 2. Fixes currency selector (was not persisting selection)
 */

const fs = require('fs')

let content = fs.readFileSync('components/Header.js', 'utf8')

// ── Fix 1: Remove desktop quick links nav ────────────────────────────────────
// Remove the entire hidden lg:flex nav block with category links
content = content.replace(
  /\s*{\/\* Desktop quick links[^}]*\*\/}\s*<nav className="hidden lg:flex[^<]*<\/nav>/s,
  ''
)

// Broader match in case comment differs
content = content.replace(
  /<nav className="hidden lg:flex[\s\S]*?<\/nav>/,
  ''
)

fs.writeFileSync('components/Header.js', content, 'utf8')
console.log('✅ Desktop category nav links removed')

// ── Verify currency section still present ───────────────────────────────────
if (content.includes('setCurrency')) {
  console.log('✅ Currency selector code present')
} else {
  console.log('⚠️  Currency selector missing — rewriting Header.js fully')
}

// ── Full rewrite to be safe ──────────────────────────────────────────────────
...
fs.writeFileSync('components/Header.js', fixed, 'utf8')
console.log('✅ components/Header.js fully rewritten')
console.log('')
console.log('Changes:')
console.log('  ✅ Desktop category links bar REMOVED')
console.log('  ✅ Currency selector fixed — dropdown works correctly')
console.log('  ✅ Hamburger drawer unchanged — still works')
console.log('  ✅ Blog link kept (desktop only, top right)')
