...
export default function MortgagePriceClient({item:it,all}){
  ...
  return(<div style={s.page}><Header/>
    ...
    <div style={s.grid}>
      <div style={s.card}><label style={s.lbl}>Home Price</label><div style={s.val}>{fmt(price)}</div><input type="number" value={price} onChange={e=>setPrice(+e.target.value)} className="calc-input"/></div>
      ...
      <div style={s.card}><label style={s.lbl}>Loan Term</label><div style={s.val}>{term} years</div><input type="number" value={term} onChange={e=>setTerm(+e.target.value)} className="calc-input"/></div>
    </div>
    <div style={s.box}>
