...
export default function FreelanceRateClient({item:it,all}){
  ...
  return(<div style={s.page}><Header/>
    ...
    <div style={s.grid}>
      <div style={s.card}><label style={s.lbl}>Target Annual Income</label><div style={s.val}>{fmt(income)}/yr</div><input type="number" value={income} onChange={e=>setIncome(+e.target.value)} style={s.sldr}/></div>
      <div style={s.card}><label style={s.lbl}>Billable Utilization</label><div style={s.val}>{util}% of hours</div><input type="number" value={util} onChange={e=>setUtil(+e.target.value)} style={s.sldr}/></div>
      <div style={s.card}><label style={s.lbl}>Hours per Week</label><div style={s.val}>{hours} hrs/wk</div><input type="number" value={hours} onChange={e=>setHours(+e.target.value)} style={s.sldr}/></div>
      <div style={s.card}><label style={s.lbl}>Weeks Working per Year</label><div style={s.val}>{weeks} weeks</div><input type="number" value={weeks} onChange={e=>setWeeks(+e.target.value)} style={s.sldr}/></div>
    </div>
    <div style={s.box}>
