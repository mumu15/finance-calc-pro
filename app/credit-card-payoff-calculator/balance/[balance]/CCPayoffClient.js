...
export default function CCPayoffClient({item:it,all}){
  ...
  return(<div style={s.page}><Header/>
    ...
    <div style={s.grid}>
      <div style={s.card}><label style={s.lbl}>Balance</label><div style={s.val}>{fmt(balance)}</div><input type="number" step="any" value={balance} onChange={e=>{setBalance(+e.target.value);setMonthly(Math.round(pmt(+e.target.value,apr,36)))}} className="calc-input"/></div>
      <div style={s.card}><label style={s.lbl}>APR</label><div style={s.val}>{apr}%</div><input type="number" step="any" value={apr} onChange={e=>setApr(+e.target.value)} className="calc-input"/></div>
      <div style={{...s.card,gridColumn:'span 2'}}><label style={s.lbl}>Monthly Payment</label><div style={s.val}>{fmt(monthly)}/mo</div><input type="number" step="any" value={monthly} onChange={e=>setMonthly(+e.target.value)} className="calc-input"/></div>
    </div>
    <div style={s.box}>
