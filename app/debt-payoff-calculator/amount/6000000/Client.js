...
export default function Client() {
  ...
  const [payment, setPayment] = useState(120000);
      ...
      <div className="bg-slate-800/50 border border-red-500/20 rounded-2xl p-6 mb-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div>
            ...
            <input type="text" inputMode="decimal" value={balance} onChange={e => setBalance(e.target.value)} className="calc-input" min="100" step="100" />
          </div>
          <div>
            <label className="block text-slate-400 text-sm font-medium mb-2">Annual Interest Rate (%)</label>
            <input type="text" inputMode="decimal" value={rate} onChange={e => setRate(e.target.value)} className="calc-input" min="0.1" max="40" step="0.1" />
          </div>
          <div>
            <label className="block text-slate-400 text-sm font-medium mb-2">Monthly Payment ($)</label>
            <input type="text" inputMode="decimal" value={payment} onChange={e => setPayment(e.target.value)} className="calc-input" min="1" step="50" />
          </div>
        </div>
