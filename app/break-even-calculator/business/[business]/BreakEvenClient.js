...
export default function BreakEvenClient({item:it,all}){
  ...
  return(<div style={s.page}><Header/>
    ...
    <div style={s.grid}>
      <div style={s.card}><label style={s.lbl}>Monthly Fixed Costs</label><div style={s.val}>{fmt(fixed)}/mo</div><input type="number" step="any" value={fixed} onChange={e=>setFixed(+e.target.value)} className="calc-input"/></div>
      <div style={s.card}><label style={s.lbl}>Price per {it.unit}</label><div style={s.val}>{fmt(price)}</div><input type="number" step="any" value={price} onChange={e=>setPrice(+e.target.value)} className="calc-input"/></div>
      <div style={{...s.card,gridColumn:'span 2'}}><label style={s.lbl}>Variable Cost %</label><div style={s.val}>{varPct}% per {it.unit}</div><input type="number" step="any" value={varPct} onChange={e=>setVarPct(+e.target.value)} className="calc-input"/></div>
    </div>
    <div style={s.box}>
