      ...
      <main className="max-w-4xl mx-auto px-4 py-10">
        <div className="text-center mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">
            Home Affordability on <span className="text-emerald-400">$1.2M Income</span>
          </h1>
        ...
        <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input type="text" inputMode="decimal" value={income} onChange={e => setIncome(Number(e.target.value))}
            ...
            <div>
              ...
              <input type="text" inputMode="decimal" value={rate} step="0.1" onChange={e => setRate(Number(e.target.value))}
                className="w-full bg-slate-900 border border-slate-600 rounded-lg px-4 py-3 text-white focus:border-emerald-500 focus:outline-none" />
            </div>
            <div>
              <label className="text-slate-400 text-sm mb-2 block">Loan Term (years)</label>
              <input type="text" inputMode="decimal" value={term} onChange={e => setTerm(Number(e.target.value))}
                className="w-full bg-slate-900 border border-slate-600 rounded-lg px-4 py-3 text-white focus:border-emerald-500 focus:outline-none" />
            </div>
            <div>
              <label className="text-slate-400 text-sm mb-2 block">Down Payment (%)</label>
              <input type="text" inputMode="decimal" value={downPct} onChange={e => setDownPct(Number(e.target.value))}
                className="w-full bg-slate-900 border border-slate-600 rounded-lg px-4 py-3 text-white focus:border-emerald-500 focus:outline-none" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-6">
          <h2 className="text-white font-bold text-lg mb-4">Home Buying on $1.2M/Year</h2>
          <p className="text-slate-400 text-sm leading-relaxed">
            With an annual income of $1.2M, you can typically afford a home priced up to {fmt(maxHome)},
