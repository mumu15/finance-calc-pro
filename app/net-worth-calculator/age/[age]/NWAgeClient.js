...
export default function NWAgeClient({item:it,all}){
  ...
  return(<div style={s.page}><Header/>
    ...
    <div style={s.grid}>
      <div style={s.card}><label style={s.lbl}>Total Assets</label><div style={s.val}>{fmt(assets)}</div><input type="number" step="any" value={assets} onChange={e=>setAssets(+e.target.value)} className="calc-input"/></div>
      <div style={s.card}><label style={s.lbl}>Total Liabilities</label><div style={s.val}>{fmt(liabilities)}</div><input type="number" step="any" value={liabilities} onChange={e=>setLiab(+e.target.value)} className="calc-input"/></div>
    </div>
    <div style={s.box}>
