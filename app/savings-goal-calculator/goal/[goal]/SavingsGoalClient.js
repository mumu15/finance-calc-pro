...
export default function SavingsGoalClient({ item: g, all }) {
  ...
  return (
    ...
    { name: 'Savings Goal Calculator', url: 'https://freefincalc.net/savings-goal-calculator' }
      ...
      <div style={st.wrap}>
        ...
        <div style={st.grid}>
          <div style={st.card}>
            ...
            <input type="number" value={target} onChange={e => setTarget(+e.target.value)} className="calc-input" />
          </div>
          <div style={st.card}>
            <label style={st.lbl}>Timeframe (months)</label>
            <div style={st.val}>{months} mo ({(months/12).toFixed(1)} yrs)</div>
            <input type="number" value={months} onChange={e => setMonths(+e.target.value)} className="calc-input" />
          </div>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"Home","item":"https://freefincalc.net"},{"@type":"ListItem","position":2,"name":"Savings Goal Calculator","item":"https://freefincalc.net/savings-goal-calculator"}]})}} />
