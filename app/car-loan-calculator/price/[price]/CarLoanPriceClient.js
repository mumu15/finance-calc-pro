...
export default function CarLoanPriceClient({item:it,all}){
  ...
  return(<div style={s.page}><Header/>
    ...
    <div style={s.grid}>
      <div style={s.card}><label style={s.lbl}>Vehicle Price</label><div style={s.val}>{fmt(price)}</div><input type="text" inputMode="decimal" value={price} onChange={e=>setPrice(+e.target.value)} className="calc-input"/></div>
      <div style={s.card}><label style={s.lbl}>Down Payment</label><div style={s.val}>{down}% — {fmt(price*down/100)}</div><input type="text" inputMode="decimal" value={down} onChange={e=>setDown(+e.target.value)} className="calc-input"/></div>
      <div style={s.card}><label style={s.lbl}>APR Rate</label><div style={s.val}>{rate}%</div><input type="text" inputMode="decimal" value={rate} onChange={e=>setRate(+e.target.value)} className="calc-input"/></div>
      <div style={s.card}><label style={s.lbl}>Loan Term (months)</label><div style={s.val}>{term} months</div><input type="text" inputMode="decimal" value={term} onChange={e=>setTerm(+e.target.value)} className="calc-input"/></div>
    </div>
    <div style={s.box}>
