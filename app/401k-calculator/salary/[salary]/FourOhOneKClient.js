...
export default function FourOhOneKClient({item:it,all}){
  ...
  return(<div style={s.page}><Header/>
    ...
    <div style={s.grid}>
      <div style={s.card}><label style={s.lbl}>Annual Salary</label><div style={s.val}>{fmt(salary)}/yr</div><input type="number" step="any" value={salary} onChange={e=>setSalary(+e.target.value)} className="calc-input"/></div>
      <div style={s.card}><label style={s.lbl}>Your Contribution</label><div style={s.val}>{contrib}% — {fmt(salary*contrib/100/12)}/mo</div><input type="number" step="any" value={contrib} onChange={e=>setContrib(+e.target.value)} className="calc-input"/></div>
      <div style={s.card}><label style={s.lbl}>Employer Match</label><div style={s.val}>{match}% — {fmt(salary*Math.min(match,contrib)/100/12)}/mo</div><input type="number" step="any" value={match} onChange={e=>setMatch(+e.target.value)} className="calc-input"/></div>
      <div style={s.card}><label style={s.lbl}>Years to Retirement</label><div style={s.val}>{years} years</div><input type="number" step="any" value={years} onChange={e=>setYears(+e.target.value)} className="calc-input"/></div>
    </div>
    <div style={s.box}>
