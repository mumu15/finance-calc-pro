      ...
      <div className="bg-slate-800/50 border border-emerald-500/20 rounded-2xl p-6 mb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <input type="range" value={principal} onChange={e => setPrincipal(e.target.value)} className="w-full bg-slate-900 border border-slate-600 rounded-xl px-4 py-3 text-white text-lg font-semibold focus:outline-none focus:border-emerald-400 transition-colors" min="0" step="1000" />
          </div>
          <div>
            <label className="block text-slate-400 text-sm font-medium mb-2">Monthly Contribution ($)</label>
            <input type="range" value={monthly} onChange={e => setMonthly(e.target.value)} className="w-full bg-slate-900 border border-slate-600 rounded-xl px-4 py-3 text-white text-lg font-semibold focus:outline-none focus:border-emerald-400 transition-colors" min="0" step="100" />
          </div>
          <div>
            <label className="block text-slate-400 text-sm font-medium mb-2">Annual Return (%)</label>
            <input type="range" value={rate} onChange={e => setRate(e.target.value)} className="w-full bg-slate-900 border border-slate-600 rounded-xl px-4 py-3 text-white text-lg font-semibold focus:outline-none focus:border-emerald-400 transition-colors" min="0.1" max="50" step="0.5" />
          </div>
          <div>
            <label className="block text-slate-400 text-sm font-medium mb-2">Time Period (years)</label>
            <input type="range" value={years} onChange={e => setYears(e.target.value)} className="w-full bg-slate-900 border border-slate-600 rounded-xl px-4 py-3 text-white text-lg font-semibold focus:outline-none focus:border-emerald-400 transition-colors" min="1" max="50" step="1" />
          </div>
        </div>
