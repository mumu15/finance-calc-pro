...
function buildUrlList() {
  ...
  // Match both formats: url: "/path" and "url": "/path"
...
function checkUrl(urlPath) {
  return new Promise((resolve) => {
    ...
    req.on('timeout', () => { req.destroy(); results.timeout.push({ url: urlPath, status: 'TIMEOUT' }); checked++; resolve(); });
    req.on('error', () => { results.other.push({ url: urlPath, status: 'ERROR' }); checked++; resolve(); });
  });
}
...
async function main() {
  ...
  if (results.notFound.length > 0) {
    ...
    results.notFound.forEach(r => console.log(`    ${DOMAIN}${r.url}`));
    console.log('');
  }
  if (results.serverError.length > 0) {
    console.log('  SERVER ERRORS:');
    results.serverError.forEach(r => console.log(`    ${DOMAIN}${r.url} (${r.status})`));
    console.log('');
  }
  if (results.redirect.length > 0) {
    console.log('  REDIRECTS:');
    results.redirect.slice(0, 20).forEach(r => console.log(`    ${r.url} -> ${r.location}`));
    if (results.redirect.length > 20) console.log(`    ... and ${results.redirect.length - 20} more`);
    console.log('');
  }
  ...
  checks.push({ pass: results.ok.length >= 50, msg: `${results.ok.length} live pages` });
