/**
 * node fix_currency_geo.js
 * Fixes automatic currency detection from user's location
 * Uses ipapi.co (free, no API key needed) to detect country → currency
 */
const fs = require('fs')

const currencyContextCode = `'use client'
...
const CURRENCIES = [
  ...
  { code: 'AED', symbol: 'د.إ',flag: '🇦🇪', name: 'UAE Dirham',         locale: 'ar-AE', defaults: { home: 1500000, loan: 100000, income: 180000 } },
  { code: 'SAR', symbol: '﷼',  flag: '🇸🇦', name: 'Saudi Riyal',        locale: 'ar-SA', defaults: { home: 1200000, loan: 80000, income: 150000 } },
  { code: 'PKR', symbol: '₨',  flag: '🇵🇰', name: 'Pakistani Rupee',    locale: 'en-PK', defaults: { home: 15000000, loan: 1000000, income: 1500000 } },
  { code: 'NGN', symbol: '₦',  flag: '🇳🇬', name: 'Nigerian Naira',     locale: 'en-NG', defaults: { home: 50000000, loan: 5000000, income: 8000000 } },
  ...
  { code: 'JPY', symbol: '¥',  flag: '🇯🇵', name: 'Japanese Yen',       locale: 'ja-JP', defaults: { home: 50000000, loan: 3000000, income: 5000000 } },
  { code: 'CNY', symbol: '¥',  flag: '🇨🇳', name: 'Chinese Yuan',       locale: 'zh-CN', defaults: { home: 3000000, loan: 200000, income: 200000 } },
...
export function CurrencyProvider({ children }) {
  ...
  useEffect(() => {
    ...
    // 2. Try geolocation via ipapi.co (free, no key needed)
    fetch('https://ipapi.co/json/')
      ...
      .catch(() => {
        ...
        if (lang.includes('en-GB')) setCurrencyState(CURRENCIES.find(c => c.code === 'GBP'))
        else if (lang.includes('en-AU')) setCurrencyState(CURRENCIES.find(c => c.code === 'AUD'))
        else if (lang.includes('en-IN')) setCurrencyState(CURRENCIES.find(c => c.code === 'INR'))
        else if (lang.includes('de') || lang.includes('fr') || lang.includes('it') || lang.includes('es')) {
          setCurrencyState(CURRENCIES.find(c => c.code === 'EUR'))
        }
        setDetected(true)
      })
  }, [])
...
fs.writeFileSync('context/CurrencyContext.js', currencyContextCode, 'utf8')
console.log('✅ CurrencyContext.js updated with geolocation detection')
console.log('')
console.log('Detection order:')
console.log('1. localStorage (remembers user preference)')
console.log('2. ipapi.co geolocation (detects country → currency)')
console.log('3. Browser language fallback')
