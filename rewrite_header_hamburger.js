/**
 * FreeFinCalc.net — Header with Mobile-Optimized Hamburger Menu
 * node rewrite_header_hamburger.js
 */
      ...
      <header style={{
        ...
        <div className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between gap-3">
          ...
          <nav className="hidden lg:flex items-center gap-1 flex-1 px-4">
            {NAV.map(cat => (
              <a key={cat.label} href={'/#' + cat.label.toLowerCase().replace(/[^a-z]/g,'-')}
                className="px-3 py-1.5 rounded-lg text-xs font-semibold text-slate-400 hover:text-white hover:bg-white/5 transition-all whitespace-nowrap">
                {cat.icon} {cat.label}
              </a>
            ))}
            <a href="/blog"
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="relative">
              <button onClick={() => setCurOpen(!curOpen)}
                ...
                <span style={{fontSize:'8px', opacity:0.6}}>{curOpen ? '▲' : '▼'}</span>
              </button>
              {curOpen && (
                <>
                  <div className="fixed inset-0 z-40" onClick={() => setCurOpen(false)} />
                  <div className="absolute right-0 top-full mt-2 rounded-2xl z-50 overflow-hidden"
                    style={{
                      background: 'rgba(15,18,30,0.99)',
                    ...
                    {CURRENCIES.map(cur => (
                      <button key={cur.code}
                        ...
                        <span className="font-bold w-7 shrink-0">{cur.symbol}</span>
            ...
            <button
              ...
              style={{
                background: menuOpen ? 'rgba(240,200,66,0.15)' : 'rgba(255,255,255,0.06)',
        ...
        <>
          ...
          <div
            ...
            <div className="flex items-center justify-between px-5 py-4 shrink-0"
              ...
              <div>
                <div className="text-white font-bold text-base">All Calculators</div>
                <div className="text-slate-500 text-xs">124 free financial tools</div>
              </div>
            ...
            {/* Quick links */}
            <div className="px-4 pt-4 pb-3 flex flex-wrap gap-2 shrink-0">
              <a href="/" onClick={() => setMenuOpen(false)}
                className="px-3 py-1.5 rounded-lg text-xs font-semibold text-slate-300 transition-all"
              ...
              {NAV.map((cat, i) => (
                <div key={cat.label} className="mb-2">
                  <button
                    ...
                    style={{
                      background: expanded === i ? 'rgba(240,200,66,0.1)' : 'rgba(255,255,255,0.04)',
                      border: expanded === i ? '1px solid rgba(240,200,66,0.2)' : '1px solid rgba(255,255,255,0.06)',
                    }}>
                    ...
                    <span className="text-slate-500 text-xs font-bold transition-transform duration-200"
                      ...
                      <div className="grid grid-cols-2 gap-1.5">
                        {cat.tools.map(tool => (
                          <a
                            ...
                            href={tool.href}
                            onClick={() => setMenuOpen(false)}
                            className="px-3 py-2.5 rounded-xl text-xs font-medium text-slate-300 hover:text-white transition-all"
                            style={{
            ...
            <div className="px-5 py-4 shrink-0"
              ...
              <div className="flex items-center justify-between">
                <span className="text-slate-500 text-xs">© 2026 FreeFinCalc.net</span>
...
console.log(`
════════════════════════════════════════════════════
  HEADER REWRITTEN — Hamburger Style
════════════════════════════════════════════════════
  ✅ components/Header.js

  Top bar:
  - Logo left
  - Desktop: category quick links (text only)
  - Currency picker
  - Hamburger button (animated 3-line → X)

  Hamburger drawer (slides from right):
  - 124 calculators in 8 category accordions
  - Tap category to expand tools grid
  - 2-column tool grid inside each category
