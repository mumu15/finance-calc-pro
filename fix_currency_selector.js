/**
 * FreeFinCalc.net — Fix currency selection not working
 * 
 * Bugs found:
 * 1. CurrencySelector groups by c.region — but CURRENCY_DATA has no region field
 *    → all currencies hidden in default (non-search) view
 * 2. Header.js uses its own private CURRENCIES array (out of sync)
 * 3. Need region field on every currency for CurrencySelector to work
 * 
 * node fix_currency_selector.js
 */

const fs = require('fs')

// ─── 1. Fix CurrencyContext.js — add region field ───────────────────────────
...
// ─── 2. Fix Header.js — remove private CURRENCIES, use context ──────────────
// Read current Header.js and patch just the currency-related parts
let header = fs.readFileSync('components/Header.js', 'utf8')

// Remove the private CURRENCIES array at top of Header.js
header = header.replace(
  /const CURRENCIES = \[\s*[\s\S]*?\]\n\n/,
  "import { CURRENCIES } from './CurrencyContext'\n\n"
)

// Fix the import line — add CURRENCIES to the useCurrency import
header = header.replace(
  `import { useCurrency } from './CurrencyContext'`,
  `import { useCurrency, CURRENCIES } from './CurrencyContext'`
)

// Remove the duplicate import if the replace above added it twice
header = header.replace(
  `import { CURRENCIES } from './CurrencyContext'\n\nimport { useCurrency, CURRENCIES } from './CurrencyContext'`,
  `import { useCurrency, CURRENCIES } from './CurrencyContext'`
)

// Fix activeCur — Header's local CURRENCIES had {code,symbol,label}, context has {code,symbol,name,...}
// The dropdown uses cur.label — patch to use cur.name
header = header.replace(/cur\.label/g, 'cur.name')

fs.writeFileSync('components/CurrencyContext.js', contextCode, 'utf8')
fs.writeFileSync('components/Header.js', header, 'utf8')

console.log('✅ components/CurrencyContext.js — region field added to all currencies')
console.log('✅ components/Header.js — uses shared CURRENCIES from context, cur.name fixed')
console.log('')
console.log('Bugs fixed:')
console.log('  1. CurrencySelector showed empty list (c.region was undefined)')
console.log('  2. Header had its own CURRENCIES array — now uses shared one')
console.log('  3. Header used cur.label — now uses cur.name')
console.log('')
console.log('Deploy:')
console.log('  git add components/CurrencyContext.js components/Header.js')
console.log('  git commit -m "fix: currency selector region grouping + shared CURRENCIES array"')
